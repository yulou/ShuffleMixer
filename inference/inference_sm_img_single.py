...
def main(args):
    ...
    all_model_paths = [os.path.join(yulou_model_paths, item) for item in all_model_paths if os.path.isfile(os.path.join(yulou_model_paths, item))]
    for single_model_path in all_model_paths:
        single_model_name, _ = os.path.splitext(os.path.basename(single_model_path))
        save_path = osp.join(args.save_path, single_model_name)
        ...
        model.load_state_dict(torch.load(single_model_path)['params'], strict=True)
        model.eval()
        ...
        for img in single_path:
            ...
            if args.save_results:
                #print(os.path.join(dataset_result_path, img_name+ext))
                #print(img_E.shape)
                util.imsave(img_E, os.path.join(dataset_result_path, img_name+ext))
